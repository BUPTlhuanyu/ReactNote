...
使用 element 作为起点，克隆并返回一个新的 React 元素。 所产生的元素的props由原始元素的 props被新的 props 浅层合并而来，并且最终合并后的props的属性为undefined，就用element.type.defaultProps也就是默认props值进行设置。 新的children子元素将取代现有的子元素， key 和 ref 将被保留。

返回的元素相当于：

	<element.type {...element.props} {...props}>{children}</element.type>

其源码与createElement类似，此处不赘述。
